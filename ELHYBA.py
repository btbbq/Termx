...
async def generator_and_about(app,m):
    ...
    if m.text == "بـايـروجـرام":
        ...
        await rep.delete()
        await c.send_message('me', f'تم استخراج جلسة بايروجرام بواسطه @BDthon {v2} هذه الجلسة\n\n`{string_session}`')
        await c.disconnect()
    ...
    if m.text == "تـيـلـيـثـون":
        ...
        await rep.delete()
        await c.send_message('me', f'تم استخراج جلسة تـيـلـيـثـون بواسطه @BDthon {v2} هذه الجلسة \n\n`{string_session}`')
        await c.disconnect()
